...
job_name = 'JSCC_'+args.channel_mode+'_coop_'+str(args.is_coop)+'_relay_' + args.relay_mode +'_dataset_'+str(args.dataset)+'_cout_'+str(args.cout)+\
            '_sr_link_' + str(args.sr_link) + '_sd_link_' + str(args.sd_link) + '_rd_link_' + str(args.rd_link) + '_alpha_' + str(args.alpha) +'_is_adapt_'+ str(args.adapt)
...
regen = Regenerate_Cell(c_in=args.cout, c_feat=args.cfeat, c_out=args.cout, attn=args.adapt).to(args.device)
source_dec = DecoderCell(c_in=2*args.cout, c_feat=args.cfeat, c_out=3, attn=args.adapt).to(args.device)
jscc_model = RelayDF(args, source_enc, source_dec, regen)
...
def train_epoch(loader, model, solvers):
    ...
    with tqdm(loader, unit='batch') as tepoch:
        for _, (images, _) in enumerate(tepoch):
            ...
            solvers.zero_grad()
            output, regen = model(images, is_train = True)

            aux_loss = args.alpha*nn.MSELoss()(regen, images)
            loss = nn.MSELoss()(output, images) + aux_loss
            
            loss.backward()
            solvers.step()

            epoch_postfix['l2_loss'] = '{:.4f}'.format(loss.item())
            epoch_postfix['aux_loss'] = '{:.4f}'.format(aux_loss.item())
    ...
    with torch.no_grad():
        with tqdm(loader, unit='batch') as tepoch:
            for _, (images, _) in enumerate(tepoch):
                ...
                output, regen = model(images, is_train = False)
                #output = model(images)
                aux_loss = args.alpha*nn.MSELoss()(regen, images)
